...
const NewsList = ({ category }) => {  //NewsList.category를 한번에 입력하기 위해 {}를 사용
    const [loading, response, error] = usePromise(() => {
        ...
        return axios.get(`https://newsapi.org/v2/top-headlines?country=kr${query}&apiKey=c77f5337872a4688a1ebd82fca07dc98`,
        );
    }, [category])
    ...
    //     const fetchData = async () => { //async를 사용하는 함수 따로 선언 중복처리를 함
    //         setLoading(true);
    //         try {
    //             const query = category === 'all' ? '' : `&category=${category}`
