...
const resolvers = {
  ...
  Mutation: {
    ...
    //   await Board.update({ number: 3 }, { deleteAt: new Date() }) // 3번 게시글 삭제했다 치자! (소프트삭제) => deletedAt이 초기값이 NULL 이면? 삭제 안된거, new Date() 들어가 있으면? 삭제 된거
    // },
  },
}

const server = new ApolloServer({
  typeDefs, // typeDefs:typeDefs key와 value가 똑같아서 생략한 것임
  resolvers,
